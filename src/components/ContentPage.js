import React from 'react'

function ContentPage() {
  return (<div className="ioXnHN gBHGdR">
  <div className="sc-bdfBQB sc-dwfUuu grkcfd cRyGxJ">
    <img height="[object Object]" alt="" width="100%" src="https://live.staticflickr.com/7706/26751237322_5a52540ea3_h.jpg" className="sc-bdfBQB sc-hKgJUU sc-TmdmN lgiHMH fmxCeH" />
    <div className="sc-bdfBQB sc-fubCzh bmhDcI ejlNuV">
      <div height="100%" display="flex" className="sc-gsTEea bZcQVH style-YUv1r" id="style-YUv1r">
        <a href="/" className="sc-bdfBQB sc-jrAFXE jTSIYX DZVSn" previewlistener="true">
          <svg viewBox="0 0 201 50" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M19.1816 17.2037H5.80924V13.3593H23.0525C22.6001 11.7283 21.1422 10.2139 18.5783 10.2139H6.0606C3.1951 10.2139 1.53613 11.4371 1.53613 14.1165V16.9707C1.53613 19.3007 3.24537 20.6404 5.75897 20.6404H19.2821V24.7178H1.08368C1.53613 26.9312 2.99401 28.0962 5.60815 28.0962H19.2318C22.0973 28.0962 23.505 26.8147 23.505 24.0771V21.2229C23.505 18.7182 21.846 17.3785 19.1816 17.2037ZM46.4792 10.2721H29.99V28.1545H34.6653V21.2811H46.8311C50.1994 21.2811 52.0594 19.9414 52.0594 16.796V14.8155C52.0091 11.9031 49.8977 10.2721 46.4792 10.2721ZM47.9874 15.9805C47.9874 17.262 47.7863 17.9609 45.9765 17.9609H34.6653L34.7156 13.3011H45.7754C47.7863 13.3011 48.0377 14 48.0377 15.2233V15.9805H47.9874ZM67.5934 21.1646H57.6396L55.1763 24.1936H69.554L72.0174 28.1545H77.5473L65.6328 9.51489L62.8176 13.534L67.5934 21.1646ZM86.4957 13.3593H103.99C103.538 11.2624 101.778 10.2139 99.2648 10.2139H86.1941C83.9318 10.2139 81.7701 11.2624 81.7701 14.1165V24.1353C81.7701 26.9895 83.9318 28.038 86.1941 28.038H99.4156C102.432 28.038 103.488 27.0477 103.99 24.6595H86.4957V13.3593ZM124.501 19.3007V16.2717H110.375V28.1545H131.238V24.776H115.1V19.3007H124.501ZM110.878 10.2721H131.941V13.4176H110.878V10.2721ZM144.912 10.2721H137.572L146.219 17.6115C147.476 16.6212 148.933 15.5728 150.24 14.699L144.912 10.2721ZM155.569 19.4172C154.312 20.4074 153.056 21.5141 151.849 22.5626L158.385 28.0962H165.775L155.569 19.4172Z" fill="black">
            </path>
            <path d="M137.572 10.2721H144.912L150.24 14.699C148.933 15.5728 147.476 16.6212 146.219 17.6115L137.572 10.2721Z" fill="black">
            </path>
            <path d="M151.849 22.5626C153.056 21.5141 154.312 20.4074 155.569 19.4172L165.775 28.0962H158.385L151.849 22.5626Z" fill="black">
            </path>
            <path d="M200.915 -0.154419C160.698 2.52502 142.097 22.4461 137.925 27.1642L137.07 28.0962H144.51C164.619 4.73847 193.525 0.602813 200.915 -0.154419Z" fill="black">
            </path>
            <path d="M0.929828 38.432H1.45783V46.544H6.31783V47H0.929828V38.432ZM15.9298 38.432H16.4578V47H15.9298V38.432ZM29.5369 46.544H29.5609L32.5249 38.432H33.0529L29.8489 47H29.2369L26.0449 38.432H26.5969L29.5369 46.544ZM42.6369 38.432H48.3969V38.888H43.1649V42.32H48.0849V42.776H43.1649V46.544H48.4569V47H42.6369V38.432ZM57.5957 43.508H63.5957V43.964H57.5957V43.508ZM73.3517 38.432H73.8797V47H73.3517V38.432ZM84.3908 38.432H85.0508L90.4628 46.292H90.4868V38.432H91.0148V47H90.3908L84.9428 39.08H84.9188V47H84.3908V38.432ZM101.524 38.432H106.9V38.888H102.052V42.32H106.42V42.776H102.052V47H101.524V38.432ZM124.452 42.716C124.452 43.34 124.36 43.928 124.176 44.48C123.992 45.024 123.724 45.5 123.372 45.908C123.028 46.308 122.604 46.624 122.1 46.856C121.596 47.088 121.024 47.204 120.384 47.204C119.744 47.204 119.168 47.088 118.656 46.856C118.152 46.624 117.724 46.308 117.372 45.908C117.028 45.5 116.764 45.024 116.58 44.48C116.396 43.928 116.304 43.34 116.304 42.716C116.304 42.092 116.396 41.508 116.58 40.964C116.764 40.412 117.028 39.936 117.372 39.536C117.724 39.128 118.152 38.808 118.656 38.576C119.168 38.344 119.744 38.228 120.384 38.228C121.024 38.228 121.596 38.344 122.1 38.576C122.604 38.808 123.028 39.128 123.372 39.536C123.724 39.936 123.992 40.412 124.176 40.964C124.36 41.508 124.452 42.092 124.452 42.716ZM116.832 42.716C116.832 43.268 116.912 43.788 117.072 44.276C117.232 44.764 117.464 45.192 117.768 45.56C118.072 45.92 118.444 46.208 118.884 46.424C119.324 46.64 119.824 46.748 120.384 46.748C120.944 46.748 121.44 46.64 121.872 46.424C122.312 46.208 122.684 45.92 122.988 45.56C123.292 45.192 123.524 44.764 123.684 44.276C123.844 43.788 123.924 43.268 123.924 42.716C123.924 42.164 123.844 41.644 123.684 41.156C123.524 40.668 123.292 40.244 122.988 39.884C122.684 39.516 122.312 39.224 121.872 39.008C121.44 38.792 120.944 38.684 120.384 38.684C119.824 38.684 119.324 38.792 118.884 39.008C118.444 39.224 118.072 39.516 117.768 39.884C117.464 40.244 117.232 40.668 117.072 41.156C116.912 41.644 116.832 42.164 116.832 42.716Z" fill="black">
            </path>
          </svg>
        </a>
        <nav>
          <ul fontSize="lg" className="sc-bdfBQB sc-kEjbQP jqsqti hleEIG">
            <li className="sc-iqHYmW ijwdtC">
              <a aria-current="page" className="sc-crrszt dBJCQk active style-5G3oy" href="/" previewlistener="true" id="style-5G3oy">
                Dashboard
              </a>
            </li>
            <li className="sc-iqHYmW ijwdtC">
              <a className="sc-crrszt dBJCQk" href="/rockets" previewlistener="true">
                Rockets
              </a>
            </li>
          </ul>
        </nav>
      </div>
    </div>
    <div width="100%" height="[object Object]" display="flex" className="sc-gsTEea kWnxzG">
      <main display="flex" className="sc-gsTEea sc-jHVedQ ioXnHN hLhMVj">
        <div display="flex" className="sc-gsTEea gtXlbR style-sznC2" id="style-sznC2">
          <article width="[object Object]" display="flex" className="sc-gsTEea sc-kstqJO exkUxL epkVnJ">
            <div width="100%" display="flex" className="sc-gsTEea hjuMS">
              <h3 className="sc-bdfBQB sc-bqyKOL iLfceD">
                Upcoming launch
              </h3>
              <div width="100%" display="flex" className="sc-gsTEea jnMkCi">
                <div display="flex" width="50%" className="sc-gsTEea sc-hBEYId hdEHzC">
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-JLoKC" id="style-JLoKC">
                      mission name
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  jpdMpM">
                      USSF-44
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-NdqQR" id="style-NdqQR">
                      rocket
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                      Falcon Heavy
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-pBj4L" id="style-pBj4L">
                      flight number
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                      188
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-hrWac" id="style-hrWac">
                      TIME (UTC)
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  jpdMpM">
                      Nov 01, 01:41 PM
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-XciGX" id="style-XciGX">
                      links
                    </h4>
                    <ul display="flex" className="sc-gsTEea kyyQfl">
                      <ul display="flex" className="sc-gsTEea qFfjh">
                        <div target="_blank" data-tip="Wikipedia Unavailable" display="flex" className="sc-gsTEea vubGq style-T9BOx" currentitem="false" id="style-T9BOx">
                          <svg height="25" width="25" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 97.75 97.75" fill="#4a4a4a">
                            <path d="M48.875,0C21.883,0,0,21.883,0,48.875S21.883,97.75,48.875,97.75S97.75,75.867,97.75,48.875S75.867,0,48.875,0z M77.691,37.503c-2.779,6.28-11.279,26.171-16.951,39.136c-0.008,0.006-1.486-0.003-1.49-0.005l-8.945-21.069 c-3.545,6.953-7.473,14.181-10.832,21.059c-0.02,0.035-1.625,0.016-1.627-0.006c-5.135-11.986-10.459-23.893-15.621-35.87 c-1.195-2.928-5.387-7.637-8.256-7.61c0-0.34-0.016-1.099-0.02-1.558l17.682-0.002l-0.014,1.531 c-2.076,0.096-5.664,1.421-4.734,3.713c2.492,5.381,11.316,26.227,13.701,31.519c1.664-3.257,6.311-11.939,8.225-15.609 c-1.5-3.078-6.457-14.57-7.943-17.464c-1.121-1.887-3.934-2.118-6.1-2.151c0-0.483,0.025-0.855,0.016-1.518l15.543,0.048v1.412 c-2.104,0.058-4.096,0.841-3.193,2.853c2.091,4.34,3.312,7.43,5.231,11.444c0.613-1.176,3.755-7.622,5.253-11.024 c0.905-2.262-0.447-3.109-4.232-3.211c0.05-0.372,0.017-1.119,0.05-1.475l13.424,0.013l0.006,1.401 c-2.467,0.096-5.021,1.41-6.354,3.45l-6.464,13.406c0.709,1.773,6.924,15.58,7.578,17.111L74.988,36.18 c-0.951-2.497-3.984-3.055-5.17-3.082c0.008-0.398,0.01-1.005,0.012-1.512l13.951,0.04l0.02,0.07l-0.023,1.394 C80.717,33.183,78.824,34.82,77.691,37.503z">
                            </path>
                          </svg>
                        </div>
                        <div className="__react_component_tooltip tfeb8e255-65d7-4af9-ae40-aa45e8e28cd1 place-bottom type-dark" id="tfeb8e255-65d7-4af9-ae40-aa45e8e28cd1" data-id="tooltip">
                        </div>
                      </ul>
                      <ul display="flex" className="sc-gsTEea qFfjh">
                        <a href="https://youtu.be/pY628jRd6gM" target="_blank" data-tip="Youtube" display="flex" className="sc-gsTEea vubGq style-h9Lvs" currentitem="false" previewlistener="true" id="style-h9Lvs">
                          <svg height="25" width="25" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" fill="#fff">
                            <path d="m224.113281 303.960938 83.273438-47.960938-83.273438-47.960938zm0 0">
                            </path>
                            <path d="m256 0c-141.363281 0-256 114.636719-256 256s114.636719 256 256 256 256-114.636719 256-256-114.636719-256-256-256zm159.960938 256.261719s0 51.917969-6.585938 76.953125c-3.691406 13.703125-14.496094 24.507812-28.199219 28.195312-25.035156 6.589844-125.175781 6.589844-125.175781 6.589844s-99.878906 0-125.175781-6.851562c-13.703125-3.6875-24.507813-14.496094-28.199219-28.199219-6.589844-24.769531-6.589844-76.949219-6.589844-76.949219s0-51.914062 6.589844-76.949219c3.6875-13.703125 14.757812-24.773437 28.199219-28.460937 25.035156-6.589844 125.175781-6.589844 125.175781-6.589844s100.140625 0 125.175781 6.851562c13.703125 3.6875 24.507813 14.496094 28.199219 28.199219 6.851562 25.035157 6.585938 77.210938 6.585938 77.210938zm0 0">
                            </path>
                          </svg>
                        </a>
                        <div className="__react_component_tooltip t35baf934-ea14-4c8e-a210-5a3e4afe4b30 place-bottom type-dark" id="t35baf934-ea14-4c8e-a210-5a3e4afe4b30" data-id="tooltip">
                        </div>
                      </ul>
                      <ul display="flex" className="sc-gsTEea qFfjh">
                        <div target="_blank" data-tip="Reddit Unavailable" display="flex" className="sc-gsTEea vubGq style-nNIoe" currentitem="false" id="style-nNIoe">
                          <svg height="25" width="25" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" fill="#4a4a4a">
                            <path d="m309.605469 343.347656c-11.46875 11.46875-36.042969 15.5625-53.554688 15.5625-17.5625 0-42.085937-4.09375-53.554687-15.5625-2.714844-2.714844-7.066406-2.714844-9.777344 0-2.714844 2.714844-2.714844 7.066406 0 9.777344 18.175781 18.175781 53.09375 19.609375 63.332031 19.609375s45.105469-1.433594 63.335938-19.609375c2.660156-2.714844 2.660156-7.066406 0-9.777344-2.714844-2.714844-7.066407-2.714844-9.78125 0zm0 0">
                            </path>
                            <path d="m224 282.675781c0-14.695312-11.980469-26.675781-26.675781-26.675781-14.691407 0-26.675781 11.980469-26.675781 26.675781 0 14.691407 11.984374 26.675781 26.675781 26.675781 14.695312 0 26.675781-11.980468 26.675781-26.675781zm0 0">
                            </path>
                            <path d="m256 0c-141.363281 0-256 114.636719-256 256s114.636719 256 256 256 256-114.636719 256-256-114.636719-256-256-256zm148.53125 290.148438c.5625 3.6875.871094 7.425781.871094 11.214843 0 57.445313-66.867188 103.988281-149.351563 103.988281s-149.351562-46.542968-149.351562-103.988281c0-3.839843.308593-7.628906.871093-11.316406-13.003906-5.835937-22.066406-18.890625-22.066406-34.046875 0-20.582031 16.691406-37.324219 37.324219-37.324219 10.035156 0 19.097656 3.941407 25.804687 10.394531 25.90625-18.6875 61.75-30.621093 101.632813-31.644531 0-.511719 18.636719-89.292969 18.636719-89.292969.359375-1.738281 1.382812-3.226562 2.867187-4.195312 1.484375-.972656 3.277344-1.28125 5.019531-.921875l62.054688 13.207031c4.351562-8.804687 13.308594-14.898437 23.804688-14.898437 14.746093 0 26.675781 11.929687 26.675781 26.675781s-11.929688 26.675781-26.675781 26.675781c-14.285157 0-25.855469-11.265625-26.519532-25.394531l-55.554687-11.828125-16.996094 80.027344c39.167969 1.378906 74.34375 13.257812 99.839844 31.691406 6.707031-6.5 15.820312-10.496094 25.90625-10.496094 20.636719 0 37.324219 16.691407 37.324219 37.324219 0 15.257812-9.164063 28.3125-22.117188 34.148438zm0 0">
                            </path>
                            <path d="m314.675781 256c-14.695312 0-26.675781 11.980469-26.675781 26.675781 0 14.691407 11.980469 26.675781 26.675781 26.675781 14.691407 0 26.675781-11.984374 26.675781-26.675781 0-14.695312-11.980468-26.675781-26.675781-26.675781zm0 0">
                            </path>
                          </svg>
                        </div>
                        <div className="__react_component_tooltip te4b7d413-0f1e-48f3-b5db-8b66c73c7d72 place-bottom type-dark" id="te4b7d413-0f1e-48f3-b5db-8b66c73c7d72" data-id="tooltip">
                        </div>
                      </ul>
                    </ul>
                  </div>
                </div>
                <div display="flex" width="50%" className="sc-gsTEea sc-hBEYId hdEHzC">
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-oBrsP" id="style-oBrsP">
                      rocket logo
                    </h4>
                    <div width="[object Object]" height="[object Object]" display="flex" className="sc-gsTEea crnfrR">
                      <svg xmlns="http://www.w3.org/2000/svg" width="72" height="50.61" viewBox="0 0 64.917 45.619">
                        <path d="M54.296 29.82a3.876 3.876 0 01-.072-.397c-.121-.869-.829-1.74-1.804-2.218-1.029-.506-2.29-.743-5.435-1.02-.685-.06-1.267-.13-1.293-.157-.084-.084.385-.509.899-.814.272-.163.495-.319.496-.347 0-.03-.22-.068-.49-.086-.342-.023-.83-.16-1.595-.448-.606-.229-1.31-.458-1.562-.51l-.46-.094.283-.216c.155-.119.283-.246.283-.283 0-.096-.914-.24-1.502-.237-.278.002-.771.05-1.096.109-.325.057-.601.095-.614.084-.054-.046 1.192-.52 1.69-.642.314-.077.865-.132 1.325-.134.85-.002 1.21.079 2.432.55.386.148.722.248.748.222.094-.093-.368-.788-.686-1.032-.528-.406-.945-.46-3.557-.456-4.446.006-9.036.631-11.848 1.614-.58.203-.778.238-1.095.197-1.488-.197-4.732-1.83-8.824-4.446-2.1-1.343-7.643-5.158-8.03-5.527-.043-.042.382.142.945.408 6.543 3.093 13.984 5.207 21.728 6.172 2.813.351 5.311.532 9.191.667 1.588.055 1.985.092 2.145.196.272.179.95 1.016 1.736 2.142l.665.953.5.052c1.655.17 3.575 1.056 4.602 2.125.667.696.915 1.24.916 2.018 0 .5-.038.671-.25 1.121-.252.537-.316.612-.371.433zm-32.72-3.762c-2.16-1.29-5.375-3.944-8.144-6.725C8.711 14.591 5.006 9.326 2.89 4.35 2.4 3.197 1.962 2.04 2.003 2c.014-.014.148.263.297.616C2.88 4 4.292 6.33 5.603 8.072c1.52 2.021 3.793 4.474 5.863 6.328 3.97 3.555 8.71 6.754 13.858 9.352.842.425 1.53.79 1.53.813 0 .049-4.414 1.89-4.53 1.89-.046 0-.382-.18-.747-.397z" fill="#eb1747">
                        </path>
                        <path d="M22.527 37.24v-6.378l6.671.003 6.672.003-.453.428c-.725.684-.608.669-5.347.669h-4.08v3.306h3.543c2.302 0 3.543.028 3.543.079 0 .156-.57.703-.896.861-.32.155-.468.162-3.262.162H25.99v4.38l-.298.494c-.73 1.207-1.79 2.04-2.949 2.32l-.216.052zM49.612 43.339c-.868-.379-1.659-1.022-2.263-1.843l-.601-.817v-9.76h3.399v6.323c0 3.477-.015 6.317-.034 6.311a23.912 23.912 0 01-.5-.214zm-12.518-6.981v-5.575h3.263v4.487h2.38c1.309 0 2.38.046 2.38.102 0 .056-.23.286-.51.51-.504.403-.536.409-2.38.452l-1.87.045v5.554h-3.263z" fill="#f5f5f5">
                        </path>
                      </svg>
                    </div>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-bEtXc" id="style-bEtXc">
                      launchpad
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                      KSC LC 39A
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </article>
          <article width="[object Object]" display="flex" className="sc-gsTEea sc-kstqJO exkUxL epkVnJ">
            <div width="100%" display="flex" className="sc-gsTEea hjuMS">
              <h3 className="sc-bdfBQB sc-bqyKOL iLfceD">
                Previous launch
              </h3>
              <div width="100%" display="flex" className="sc-gsTEea jnMkCi">
                <div display="flex" width="50%" className="sc-gsTEea sc-hBEYId hdEHzC">
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-5oie1" id="style-5oie1">
                      mission name
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  jpdMpM">
                      Crew-5
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-Ojb9z" id="style-Ojb9z">
                      rocket
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                      Falcon 9
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-vwehq" id="style-vwehq">
                      flight number
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                      187
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-IgH3B" id="style-IgH3B">
                      TIME (UTC)
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  jpdMpM">
                      Oct 05, 04:00 PM
                    </p>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-pbIk6" id="style-pbIk6">
                      links
                    </h4>
                    <ul display="flex" className="sc-gsTEea kyyQfl">
                      <ul display="flex" className="sc-gsTEea qFfjh">
                        <a href="https://en.wikipedia.org/wiki/SpaceX_Crew-5" target="_blank" data-tip="Wikipedia" display="flex" className="sc-gsTEea vubGq style-nGiWs" currentitem="false" previewlistener="true" id="style-nGiWs">
                          <svg height="25" width="25" version="1.1" id="Capa_1" x="0px" y="0px" viewBox="0 0 97.75 97.75" fill="#fff">
                            <path d="M48.875,0C21.883,0,0,21.883,0,48.875S21.883,97.75,48.875,97.75S97.75,75.867,97.75,48.875S75.867,0,48.875,0z M77.691,37.503c-2.779,6.28-11.279,26.171-16.951,39.136c-0.008,0.006-1.486-0.003-1.49-0.005l-8.945-21.069 c-3.545,6.953-7.473,14.181-10.832,21.059c-0.02,0.035-1.625,0.016-1.627-0.006c-5.135-11.986-10.459-23.893-15.621-35.87 c-1.195-2.928-5.387-7.637-8.256-7.61c0-0.34-0.016-1.099-0.02-1.558l17.682-0.002l-0.014,1.531 c-2.076,0.096-5.664,1.421-4.734,3.713c2.492,5.381,11.316,26.227,13.701,31.519c1.664-3.257,6.311-11.939,8.225-15.609 c-1.5-3.078-6.457-14.57-7.943-17.464c-1.121-1.887-3.934-2.118-6.1-2.151c0-0.483,0.025-0.855,0.016-1.518l15.543,0.048v1.412 c-2.104,0.058-4.096,0.841-3.193,2.853c2.091,4.34,3.312,7.43,5.231,11.444c0.613-1.176,3.755-7.622,5.253-11.024 c0.905-2.262-0.447-3.109-4.232-3.211c0.05-0.372,0.017-1.119,0.05-1.475l13.424,0.013l0.006,1.401 c-2.467,0.096-5.021,1.41-6.354,3.45l-6.464,13.406c0.709,1.773,6.924,15.58,7.578,17.111L74.988,36.18 c-0.951-2.497-3.984-3.055-5.17-3.082c0.008-0.398,0.01-1.005,0.012-1.512l13.951,0.04l0.02,0.07l-0.023,1.394 C80.717,33.183,78.824,34.82,77.691,37.503z">
                            </path>
                          </svg>
                        </a>
                        <div className="__react_component_tooltip t87a03f44-d0a0-409c-ba78-fae6b748e978 place-bottom type-dark" id="t87a03f44-d0a0-409c-ba78-fae6b748e978" data-id="tooltip">
                        </div>
                      </ul>
                      <ul display="flex" className="sc-gsTEea qFfjh">
                        <a href="https://youtu.be/5EwW8ZkArL4" target="_blank" data-tip="Youtube" display="flex" className="sc-gsTEea vubGq style-4qSyh" currentitem="false" previewlistener="true" id="style-4qSyh">
                          <svg height="25" width="25" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" fill="#fff">
                            <path d="m224.113281 303.960938 83.273438-47.960938-83.273438-47.960938zm0 0">
                            </path>
                            <path d="m256 0c-141.363281 0-256 114.636719-256 256s114.636719 256 256 256 256-114.636719 256-256-114.636719-256-256-256zm159.960938 256.261719s0 51.917969-6.585938 76.953125c-3.691406 13.703125-14.496094 24.507812-28.199219 28.195312-25.035156 6.589844-125.175781 6.589844-125.175781 6.589844s-99.878906 0-125.175781-6.851562c-13.703125-3.6875-24.507813-14.496094-28.199219-28.199219-6.589844-24.769531-6.589844-76.949219-6.589844-76.949219s0-51.914062 6.589844-76.949219c3.6875-13.703125 14.757812-24.773437 28.199219-28.460937 25.035156-6.589844 125.175781-6.589844 125.175781-6.589844s100.140625 0 125.175781 6.851562c13.703125 3.6875 24.507813 14.496094 28.199219 28.199219 6.851562 25.035157 6.585938 77.210938 6.585938 77.210938zm0 0">
                            </path>
                          </svg>
                        </a>
                        <div className="__react_component_tooltip t40ce4bc4-8801-4e3c-9977-e0804a35355a place-bottom type-dark" id="t40ce4bc4-8801-4e3c-9977-e0804a35355a" data-id="tooltip">
                        </div>
                      </ul>
                      <ul display="flex" className="sc-gsTEea qFfjh">
                        <div target="_blank" data-tip="Reddit Unavailable" display="flex" className="sc-gsTEea vubGq style-NlfXQ" currentitem="false" id="style-NlfXQ">
                          <svg height="25" width="25" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg" fill="#4a4a4a">
                            <path d="m309.605469 343.347656c-11.46875 11.46875-36.042969 15.5625-53.554688 15.5625-17.5625 0-42.085937-4.09375-53.554687-15.5625-2.714844-2.714844-7.066406-2.714844-9.777344 0-2.714844 2.714844-2.714844 7.066406 0 9.777344 18.175781 18.175781 53.09375 19.609375 63.332031 19.609375s45.105469-1.433594 63.335938-19.609375c2.660156-2.714844 2.660156-7.066406 0-9.777344-2.714844-2.714844-7.066407-2.714844-9.78125 0zm0 0">
                            </path>
                            <path d="m224 282.675781c0-14.695312-11.980469-26.675781-26.675781-26.675781-14.691407 0-26.675781 11.980469-26.675781 26.675781 0 14.691407 11.984374 26.675781 26.675781 26.675781 14.695312 0 26.675781-11.980468 26.675781-26.675781zm0 0">
                            </path>
                            <path d="m256 0c-141.363281 0-256 114.636719-256 256s114.636719 256 256 256 256-114.636719 256-256-114.636719-256-256-256zm148.53125 290.148438c.5625 3.6875.871094 7.425781.871094 11.214843 0 57.445313-66.867188 103.988281-149.351563 103.988281s-149.351562-46.542968-149.351562-103.988281c0-3.839843.308593-7.628906.871093-11.316406-13.003906-5.835937-22.066406-18.890625-22.066406-34.046875 0-20.582031 16.691406-37.324219 37.324219-37.324219 10.035156 0 19.097656 3.941407 25.804687 10.394531 25.90625-18.6875 61.75-30.621093 101.632813-31.644531 0-.511719 18.636719-89.292969 18.636719-89.292969.359375-1.738281 1.382812-3.226562 2.867187-4.195312 1.484375-.972656 3.277344-1.28125 5.019531-.921875l62.054688 13.207031c4.351562-8.804687 13.308594-14.898437 23.804688-14.898437 14.746093 0 26.675781 11.929687 26.675781 26.675781s-11.929688 26.675781-26.675781 26.675781c-14.285157 0-25.855469-11.265625-26.519532-25.394531l-55.554687-11.828125-16.996094 80.027344c39.167969 1.378906 74.34375 13.257812 99.839844 31.691406 6.707031-6.5 15.820312-10.496094 25.90625-10.496094 20.636719 0 37.324219 16.691407 37.324219 37.324219 0 15.257812-9.164063 28.3125-22.117188 34.148438zm0 0">
                            </path>
                            <path d="m314.675781 256c-14.695312 0-26.675781 11.980469-26.675781 26.675781 0 14.691407 11.980469 26.675781 26.675781 26.675781 14.691407 0 26.675781-11.984374 26.675781-26.675781 0-14.695312-11.980468-26.675781-26.675781-26.675781zm0 0">
                            </path>
                          </svg>
                        </div>
                        <div className="__react_component_tooltip t91371258-7db1-42b8-b4f5-223f851ab5a6 place-bottom type-dark" id="t91371258-7db1-42b8-b4f5-223f851ab5a6" data-id="tooltip">
                        </div>
                      </ul>
                    </ul>
                  </div>
                </div>
                <div display="flex" width="50%" className="sc-gsTEea sc-hBEYId hdEHzC">
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-nqIYL" id="style-nqIYL">
                      mission patch
                    </h4>
                    <div width="[object Object]" height="[object Object]" display="flex" className="sc-gsTEea crnfrR">
                      <img src="https://images2.imgbox.com/eb/d8/D1Yywp0w_o.png" alt="Mission patch" className="sc-bdfBQB sc-hKgJUU dlkYfS" />
                    </div>
                  </div>
                  <div display="flex" className="sc-gsTEea sc-fodVek dSIRyy">
                    <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-eVNNO" id="style-eVNNO">
                      crew
                    </h4>
                    <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                      <svg version="1.1" width="25" height="30" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 60.671 60.671">
                        <ellipse cx="30.336" cy="12.097" rx="11.997" ry="12.097" style={{fill: "rgb(255, 255, 255)"}}>
                        </ellipse>
                        <path d="M35.64,30.079H25.031c-7.021,0-12.714,5.739-12.714,12.821v17.771h36.037V42.9 C48.354,35.818,42.661,30.079,35.64,30.079z" style={{fill: "rgb(255, 255, 255)"}}>
                        </path>
                      </svg>
                      <svg version="1.1" width="25" height="30" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 60.671 60.671">
                        <ellipse cx="30.336" cy="12.097" rx="11.997" ry="12.097" style={{fill: "rgb(255, 255, 255)"}}>
                        </ellipse>
                        <path d="M35.64,30.079H25.031c-7.021,0-12.714,5.739-12.714,12.821v17.771h36.037V42.9 C48.354,35.818,42.661,30.079,35.64,30.079z" style={{fill: "rgb(255, 255, 255)"}}>
                        </path>
                      </svg>
                      <svg version="1.1" width="25" height="30" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 60.671 60.671">
                        <ellipse cx="30.336" cy="12.097" rx="11.997" ry="12.097" style={{fill: "rgb(255, 255, 255)"}}>
                        </ellipse>
                        <path d="M35.64,30.079H25.031c-7.021,0-12.714,5.739-12.714,12.821v17.771h36.037V42.9 C48.354,35.818,42.661,30.079,35.64,30.079z" style={{fill: "rgb(255, 255, 255)"}}>
                        </path>
                      </svg>
                      <svg version="1.1" width="25" height="30" id="Capa_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 60.671 60.671">
                        <ellipse cx="30.336" cy="12.097" rx="11.997" ry="12.097" style={{fill: "rgb(255, 255, 255)"}}>
                        </ellipse>
                        <path d="M35.64,30.079H25.031c-7.021,0-12.714,5.739-12.714,12.821v17.771h36.037V42.9 C48.354,35.818,42.661,30.079,35.64,30.079z" style={{fill: "rgb(255, 255, 255)"}}>
                        </path>
                      </svg>
                    </p>
                  </div>
                </div>
              </div>
            </div>
          </article>
        </div>
        <div display="flex" className="sc-gsTEea gtXlbR style-5EVDU" id="style-5EVDU">
          <article width="[object Object]" display="flex" className="sc-gsTEea sc-kstqJO exkUxL epkVnJ">
            <div width="100%" display="flex" className="sc-gsTEea hjuMS">
              <h3 className="sc-bdfBQB sc-bqyKOL iLfceD">
                Launch facilities
              </h3>
              <div display="flex" className="sc-gsTEea sc-bBXrwG dwGUTk bRiMKL">
                <div className="sc-bdfBQB cqVhKu">
                  <div className="sc-bdfBQB sc-iJuVqt fNhnba">
                    <div display="flex" className="sc-gsTEea sc-cxFLGX YynDh">
                      <div display="flex" className="sc-gsTEea sc-lmoMya ioXnHN">
                        <div display="flex" className="sc-gsTEea sc-fodVek cGqifg">
                          <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-sglFL" id="style-sglFL">
                            LC-39A &amp; SLC-40
                          </h4>
                          <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                            Cape Canaveral
                          </p>
                        </div>
                      </div>
                      <div display="flex" className="sc-gsTEea sc-lmoMya ioXnHN">
                        <div display="flex" className="sc-gsTEea sc-fodVek foOCE">
                          <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-MgAG7" id="style-MgAG7">
                            region
                          </h4>
                          <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                            Florida
                          </p>
                        </div>
                      </div>
                    </div>
                    <div display="flex" className="sc-gsTEea sc-cxFLGX YynDh">
                      <div width="30%;" display="flex" className="sc-gsTEea sc-fodVek bCzmGD">
                        <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-zMWSX" id="style-zMWSX">
                          temp
                        </h4>
                        <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                          11°C
                        </p>
                      </div>
                      <div width="30%;" display="flex" className="sc-gsTEea sc-fodVek iNbijr">
                        <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-OKUzo" id="style-OKUzo">
                          weather
                        </h4>
                        <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                          Clear
                        </p>
                      </div>
                      <div width="30%;" display="flex" className="sc-gsTEea sc-fodVek chKEFp">
                        <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-HPnSg" id="style-HPnSg">
                          wind
                        </h4>
                        <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                          2 m/s
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <img src="https://live.staticflickr.com/65535/51134975562_11a63a62d7_h.jpg" className="sc-bdfBQB sc-hKgJUU sc-iwyWTf  kqJPCn" />
              </div>
              <div display="flex" className="sc-gsTEea sc-bBXrwG djpdJj bRiMKL">
                <div className="sc-bdfBQB cqVhKu">
                  <div className="sc-bdfBQB sc-iJuVqt fNhnba">
                    <div display="flex" className="sc-gsTEea sc-cxFLGX YynDh">
                      <div display="flex" className="sc-gsTEea sc-lmoMya ioXnHN">
                        <div display="flex" className="sc-gsTEea sc-fodVek cGqifg">
                          <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-pbzcC" id="style-pbzcC">
                            Starship Launch Facility
                          </h4>
                          <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                            Starbase Boca Chica
                          </p>
                        </div>
                      </div>
                      <div display="flex" className="sc-gsTEea sc-lmoMya ioXnHN">
                        <div display="flex" className="sc-gsTEea sc-fodVek foOCE">
                          <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-7oRjM" id="style-7oRjM">
                            region
                          </h4>
                          <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                            Texas
                          </p>
                        </div>
                      </div>
                    </div>
                    <div display="flex" className="sc-gsTEea sc-cxFLGX YynDh">
                      <div width="30%;" display="flex" className="sc-gsTEea sc-fodVek bCzmGD">
                        <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-7eoqq" id="style-7eoqq">
                          temp
                        </h4>
                        <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                          19°C
                        </p>
                      </div>
                      <div width="30%;" display="flex" className="sc-gsTEea sc-fodVek iNbijr">
                        <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-v34zp" id="style-v34zp">
                          weather
                        </h4>
                        <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                          Clouds
                        </p>
                      </div>
                      <div width="30%;" display="flex" className="sc-gsTEea sc-fodVek chKEFp">
                        <h4 color="transparentWhite" fontWeight="normal" fontSize="xxs" className="sc-bdfBQB sc-bkzYnD kreYWV kwkEfR style-yLP5U" id="style-yLP5U">
                          wind
                        </h4>
                        <p className="sc-bdfBQB sc-fFucqa  bSgtco">
                          6 m/s
                        </p>
                      </div>
                    </div>
                  </div>
                </div>
                <img src="https://live.staticflickr.com/65535/50906488116_c44db82fc1_k.jpg" className="sc-bdfBQB sc-hKgJUU sc-iwyWTf  kqJPCn" />
              </div>
              <div display="flex" className="sc-gsTEea sc-idOiZg kmZhta">
                <button display="flex" className="sc-gsTEea sc-dmlqKv iyUgRF hBLSDI">
                  <div display="flex" className="sc-gsTEea sc-kfzBvY ioXnHN hFFDGJ">
                    <svg version="1.1" width="18px" height="18px" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px" viewBox="0 0 512 512" fill="#fff">
                      <path d="M256,0C114.837,0,0,114.837,0,256s114.837,256,256,256s256-114.837,256-256S397.163,0,256,0z M335.083,271.083 L228.416,377.749c-4.16,4.16-9.621,6.251-15.083,6.251c-5.461,0-10.923-2.091-15.083-6.251c-8.341-8.341-8.341-21.824,0-30.165 L289.835,256l-91.584-91.584c-8.341-8.341-8.341-21.824,0-30.165s21.824-8.341,30.165,0l106.667,106.667 C343.424,249.259,343.424,262.741,335.083,271.083z">
                      </path>
                    </svg>
                  </div>
                  <p className="sc-bdfBQB sc-hHfuMS dsoCpj">
                    more
                  </p>
                </button>
              </div>
            </div>
          </article>
          <article width="[object Object]" display="flex" className="sc-gsTEea sc-kstqJO exkUxL epkVnJ">
            <div width="100%" display="flex" className="sc-gsTEea hjuMS">
              <h3 className="sc-bdfBQB sc-bqyKOL iLfceD">
                Starlink
              </h3>
              <div display="flex" className="sc-gsTEea sc-giImIA ioXnHN kfubSn">
                <button display="flex" className="sc-gsTEea sc-ezredP ioXnHN inJoyt">
                  <svg width="150" height="150" viewBox="0 0 714 714" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M180.612 492.891L219.292 465.505L208.912 450.846L170.233 478.232C162.847 474.835 154.1 476.822 148.907 483.077C143.715 489.332 143.37 498.295 148.067 504.93C152.765 511.564 161.334 514.218 168.959 511.397C176.584 508.576 181.364 500.986 180.612 492.891V492.891Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M485.25 530.297L500.82 552.287C503.686 556.335 509.291 557.293 513.339 554.427L535.328 538.858L514.57 509.539L485.251 530.298L485.25 530.297Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M311.262 191.307L290.504 161.988L319.823 141.23L340.581 170.549L311.262 191.307Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M342.4 235.286L321.641 205.967L350.961 185.208L371.719 214.527L342.4 235.286Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M549.988 528.479L529.229 499.16L558.548 478.401L579.307 507.72L549.988 528.479Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M325.334 71.3024L303.345 86.8714L324.103 116.19L353.423 95.4318L337.854 73.4425C334.987 69.394 329.382 68.4365 325.334 71.3024V71.3024Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M267.284 222.445L246.525 193.126L275.844 172.368L296.603 201.687L267.284 222.445Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M531.691 409.382L510.932 380.063L540.251 359.305L561.01 388.624L531.691 409.382Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M487.711 440.521L466.953 411.202L496.272 390.443L517.03 419.762L487.711 440.521Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M443.733 471.659L422.974 442.34L452.293 421.581L473.052 450.9L443.733 471.659Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M569.199 275.853L593.517 268.485C596.28 267.646 598.471 265.531 599.406 262.8C600.342 260.068 599.908 257.054 598.238 254.698L533.626 163.442C531.959 161.086 529.26 159.674 526.373 159.649C523.486 159.624 520.763 160.988 519.055 163.315L504.026 183.805L569.199 275.853Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M518.849 484.499L498.091 455.18L527.41 434.422L548.168 463.741L518.849 484.499Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M562.829 453.361L542.07 424.042L571.389 403.283L592.148 432.602L562.829 453.361Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M474.871 515.637L454.112 486.318L483.431 465.56L504.19 494.879L474.871 515.637Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M298.421 266.424L277.663 237.105L306.982 216.346L327.741 245.665L298.421 266.424Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M355.241 160.169L334.483 130.85L363.802 110.091L384.56 139.41L355.241 160.169Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M280.124 147.329L259.366 118.01L288.685 97.2512L309.443 126.57L280.124 147.329Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M618.097 469.252L602.528 447.263L573.208 468.021L593.967 497.34L615.956 481.771C620.004 478.905 620.962 473.3 618.097 469.252V469.252Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M386.379 204.148L365.621 174.829L394.94 154.07L415.698 183.389L386.379 204.148Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M222.717 143.958C218.669 146.824 217.711 152.43 220.577 156.478L236.146 178.467L265.465 157.708L244.706 128.389L222.717 143.958Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M455.397 177.291L279.482 301.843C271.386 307.575 269.47 318.785 275.202 326.882L358.237 444.159C363.969 452.255 375.18 454.171 383.276 448.439L559.191 323.887C567.287 318.154 569.204 306.944 563.471 298.848L480.436 181.571C474.704 173.474 463.494 171.558 455.397 177.291ZM481.346 213.939L533.242 287.237C541.84 299.381 538.966 316.197 526.822 324.795L402.215 413.02C390.071 421.618 373.255 418.744 364.657 406.6L312.76 333.301C304.162 321.157 307.036 304.341 319.18 295.743L443.787 207.518C455.932 198.92 472.747 201.795 481.346 213.939V213.939Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <path d="M302.06 395.899C275.129 357.374 224.655 343.368 181.724 362.507C179.269 363.624 177.438 365.775 176.728 368.377C176.018 370.979 176.503 373.761 178.05 375.97L279.563 519.548C281.13 521.747 283.594 523.133 286.286 523.329C288.979 523.525 291.619 522.511 293.486 520.562C325.87 486.441 329.467 434.133 302.06 395.899Z" fill="rgba(255,255,255,0.4)">
                    </path>
                    <rect x="385.276" y="402.561" width="107.77" height="170.64" rx="14" transform="rotate(-125.3 385.276 402.561)" fill="rgba(255,255,255,0.4)">
                    </rect>
                    <path className="blinking-light" d="M434.234 323.492C426.18 329.194 415.028 327.288 409.325 319.234V319.234C403.623 311.18 405.529 300.028 413.583 294.325V294.325C421.637 288.623 432.789 290.529 438.492 298.583V298.583C444.194 306.637 442.288 317.789 434.234 323.492V323.492Z" fill="#00EC09">
                    </path>
                  </svg>
                </button>
                <p className="sc-bdfBQB sc-dlfnuX cMNnPs style-19nrX" id="style-19nrX">
                  There are currently 3268 active Starlink satellites on the low Earth orbit.
                </p>
              </div>/home/ankit/Desktop/w/index.html
            </div>
          </article>
        </div>
      </main>
    </div>
  </div>
</div>
  )
}

export default ContentPage
